...
def main():
    ...
    # retrieve token
    r = s.get('http://links.e-jambon.com/?do=login', verify=False)
    ...
    r = s.post('http://links.e-jambon.com/?do=login', data=pl, verify=False)
    ...
    r = s.get('http://links.e-jambon.com/?post={}'.format(url), verify=False)
    a = re.search(r'(?<=token" value=")(.*)(?=">)',r.text)
    token = a.group()
    ...
    r = s.post('http://links.e-jambon.com/?post={}'.format(url), data=d, verify=False)
